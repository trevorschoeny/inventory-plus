 ...
 * given hotbar slot: the ordered items in the cycle's display window plus
 * which index holds the "current" (in-hand) item.
 *
 ...
 * current index. This is the seam that lets one HUD serve every cycler.
 *
 * <h3>Display-order convention</h3>
 *
 * The list is in left-to-right display order. {@code highlightIndex} points
 * at the in-hand item. Sources follow the established convention:
 * <ul>
 ...
 */
public record CycleView(List<ItemStack> orderedItems, int highlightIndex) {
     ...
     * Cycler produce. Applies the shared display convention:
     * <ul>
     *   <li>2 members: {@code [current, other]}, highlight 0.</li>
     *   <li>3+ members: {@code [prev, current, next, next-next, …]}, highlight 1.</li>
     * </ul>
    ...
    public static CycleView fromVisualOrder(List<ItemStack> visualTopToBottomHotbarLast) {
        ...
        return new CycleView(out, highlight);
    }
}
